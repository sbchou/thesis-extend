...
stories <- read.table('../DATA/FOR_CORR/candid_coded.csv', header=TRUE, sep = ",")
...
stories$top_candid = factor((stories$top_candid))


### num_tweets vs. Candid Camp? ###
m = lm(num_tweets ~ top_candid, data=stories)
summary(m)
...
ggplot(data=stories, aes(top_candid,num_tweets)) +  
  geom_boxplot() 
